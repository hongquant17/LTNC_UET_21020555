...
void gameLoop()
{
    ...
    while (SDL_PollEvent(&event))
    {
        switch (event.type) {
            ...
            case SDL_MOUSEBUTTONDOWN:
            {
                if (mainMenu)
                {
                    if (event.button.button == SDL_BUTTON_LEFT && SDL_GetTicks() > 3000)
                    {
                        mainMenu = false;
    ...
    if (mainMenu)
    {
        ...
        {
            window.clear();
            window.renderCenter(0, sin(SDL_GetTicks()/100) * 2 - 4, "LTNC UET", font24, white);
            window.display();
        }
        else
        {
            ...
            window.render(545, sin(SDL_GetTicks() / 100) + 20, "Tran Hong Quan", font24, white);
    ...
    else
    {
        ...
        window.render(62, 20, ("DISTANCE: " + std::to_string(player.getScoreInt())).c_str(), font32_outline, black);
        window.render(65, 23, ("DISTANCE: " + std::to_string(player.getScoreInt())).c_str(), font32, white);
...
int main(int argc, char* args[])
{
     while (gameRunning)
     {
         ...
         if (player.getScoreInt() < 30) SDL_Delay(15);
         else if (player.getScoreInt() < 60) SDL_Delay(13);
         else SDL_Delay(11);
     }
